...
import { Users, UploadCloud, Search, Download, AlertCircle, FileText, CheckCircle2, Bot, Plus, X, Phone, Mail, User } from "lucide-react";
...
export default function ContactsLedgerPage() {
  ...
  const handleCreateContact = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newContact.phone.length < 10) return alert("El teléfono debe tener mínimo 10 dígitos.");
    if (newContact.email && !newContact.email.includes("@")) return alert("El correo debe contener un @");

    setImporting(true);
    try {
      const sys = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3002"}/api/v1/admin/companies`, { headers: { Authorization: "Bearer zohomasterkey_99_omnichat_x" }});
      const companyId = sys.data[0]?.id;
      if(!companyId) return alert("Error de empresa");

      await axios.post(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3002"}/api/inbox/contacts/import`, {
          contacts: [{ name: newContact.name, phone: newContact.phone }],
          companyId
      });
      ...
      alert("Error al crear el lead manualmente.");
    } finally {
      setImporting(false);
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6 pt-6 px-6">
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-3">
            <button onClick={() => alert("Formato requerido: Nombre,Telefono\\nJuan Perez,5215588992233")} className="hidden md:flex bg-white border-2 border-slate-200 text-slate-600 font-bold py-2.5 px-5 rounded-xl hover:bg-slate-50 transition-all items-center gap-2">
            ...
            <button 
              ...
              disabled={importing}
              className="bg-slate-800 hover:bg-slate-900 text-white font-bold py-2.5 px-5 rounded-xl shadow-lg transition-all flex items-center gap-2"
            >
            ...
            >
               <Plus className="h-5 w-5" /> Nuevo Lead
            </button>
          </div>
        </div>
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden p-6 md:p-8 relative mt-10">
               ...
               <h2 className="text-2xl font-black text-slate-800 mb-2 border-l-4 border-indigo-600 pl-3">Nuevo Lead</h2>
               <p className="text-sm text-slate-500 mb-6 font-medium">Captura un prospecto manualmente. El sistema verificará la integridad de los datos para envío a WispHub.</p>
               
               <form onSubmit={handleCreateContact} className="space-y-4">
                 <div>
                   <label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-1"><User className="h-4 w-4"/> Nombre Completo</label>
                   <input 
                     required
                     autoFocus
                     type="text" 
                     ...
                     className="w-full border-2 border-slate-200 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-medium bg-slate-50 focus:bg-white"
                     placeholder="Ej: Juan Pérez"
                   />
                 </div>
                 <div>
                   <label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-1"><Phone className="h-4 w-4"/> Celular (WhatsApp)</label>
                   <input 
                     required
                     type="text" 
                     maxLength={12}
                     ...
                     onChange={e => {
                        const val = e.target.value.replace(/\D/g, ''); // Remueve cualquier letra/símbolo, solo números
                        setNewContact({...newContact, phone: val});
                     }}
                     className="w-full border-2 border-slate-200 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-mono font-medium bg-slate-50 focus:bg-white"
                     placeholder="10 dígitos Ej: 8180000000"
                   />
                   <p className="text-[10px] text-slate-400 mt-1 font-bold">Solo números. Sin guiones ni espacios.</p>
                 </div>
                 <div>
                   <label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-1"><Mail className="h-4 w-4"/> Correo (Opcional)</label>
                   <input 
                     type="email" 
                     value={newContact.email}
                     onChange={e => setNewContact({...newContact, email: e.target.value})}
                     className={`w-full border-2 rounded-xl px-4 py-2.5 focus:ring-4 outline-none transition-all font-medium bg-slate-50 focus:bg-white ${newContact.email && !newContact.email.includes('@') ? 'border-red-400 focus:border-red-500 focus:ring-red-500/10 text-red-600' : 'border-slate-200 focus:border-indigo-500 focus:ring-indigo-500/10'}`}
                     placeholder="cliente@ejemplo.com"
                   />
                   {newContact.email && !newContact.email.includes('@') && (
                      <p className="text-[10px] text-red-500 mt-1 font-bold animate-pulse">Formato de correo inválido (Falta '@')</p>
                   )}
                 </div>
                 <button 
                   type="submit" 
                   disabled={importing || (newContact.email !== "" && !newContact.email.includes('@')) || newContact.phone.length < 10}
                   className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:text-slate-500 disabled:cursor-not-allowed text-white font-black py-3 rounded-xl mt-4 transition-all shadow-md group border border-transparent disabled:border-slate-300"
                 >
                   {importing ? "Guardando en Matrix..." : "Añadir al CRM"}
                 </button>
               </form>
            </div>
          </div>
